...
void *wav_load_data(FILE *file, wav_fmt_desc *fmt_desc) {
    ...
    fmt_desc->bytes_per_sample = header.bits_per_sample / 8;
...
size_t wav_write_data(FILE *file, wav_fmt_desc fmt_desc, void *data) {
    ...
    wav_header header = wav_header_from_fmt_desc(fmt_desc);

    size_t bytes_written = 0;

    bytes_written += fwrite(&header, 1, sizeof(wav_header), file);

    WAV_ERROR_CHECK(bytes_written != 44,
                    "Writing header data results in unexpected fwrite return");

    WAV_ERROR_CHECK(ferror(file), "Error writing WAVE header");

    bytes_written += fwrite(data, 1, header.data_size, file);

    WAV_ERROR_CHECK(bytes_written != header.data_size + 44,
                    "Writing WAVE data results in unexpected fwrite return");
